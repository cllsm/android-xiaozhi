"""
统一资源路径解析器 - 开发/打包通用（Android 版）

所有用户数据统一存放在用户数据目录下：
- Android: ~/.xiaozhi/
- Windows: C:/Users/xxx/AppData/Local/android-xiaozhi/
- macOS:   ~/Library/Application Support/android-xiaozhi/
- Linux:   ~/.local/share/android-xiaozhi/
...
def get_app_root() -> Path:
    ...
    # backend/utils/resource_finder.py → 往上 3 级
...
def get_app_name() -> str:
    ...
    直接返回 "android-xiaozhi"。
    """
    return "android-xiaozhi"
...
def get_lib_path(lib_name: str) -> Path | None:
    ...
    # 平台目录别名（mac/macos, win/windows, android）
    plat_aliases = {
        "mac": ["mac", "macos"],
        "win": ["win", "windows"],
        "linux": ["linux"],
    ...
    # 扩展名
    ext_map = {"mac": ".dylib", "win": ".dll", "linux": ".so", "android": ".so"}
