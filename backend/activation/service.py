...
from backend.utils.resource_finder import get_user_data_dir
...
class ActivationService:
    ...
    def _show_activation_info(self, data: Dict):
        ...
        # 复制验证码到剪贴板
        try:
            from backend.utils.common_utils import handle_verification_code
        ...
        # 播报验证码
        try:
            from backend.utils.activation_announcer import announce_activation_code
        ...
        async with aiohttp.ClientSession(timeout=timeout) as session:
            for attempt in range(max_retries):
                try:
                    ...
                    if attempt > 0:
                        try:
                            from backend.utils.activation_announcer import (
